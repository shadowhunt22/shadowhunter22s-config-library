...
@ApiStatus.Internal
class EnumEntryBuilder<T extends ConfigData> {
    private final ConfigManager<T> configManager;
    private final MinecraftClient client;
    private final int width;

    public EnumEntryBuilder(MinecraftClient client, ConfigManager<T> configManager, int width) {
        this.client = client;
        this.configManager = configManager;
        this.width = width;
    }

    public ConfigEntryWidget.Entry build(ConfigEntryWidget<T> parentWidget, Field field, EnumConfigOption<?> configOption) {
        ThreePartsLayoutWidget layout = new ThreePartsLayoutWidget(this.client.currentScreen);

        TextWidget textWidget = new TextWidget(150, 20, Text.translatable(configOption.getTranslationKey()), this.client.textRenderer);
        textWidget.alignLeft();
        textWidget.setX(textWidget.getX() + 15);

        ButtonWidget toggleButton = ButtonWidget.builder(configOption.getText(), button -> {
            configOption.cycle();
            this.configManager.getSerializer().setValue(field, configManager.getConfig(), configOption.getValue());
            parentWidget.update();
